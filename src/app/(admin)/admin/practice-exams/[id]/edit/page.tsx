// src/app/(admin)/admin/practice-exams/[id]/edit/page.tsx
'use client'
...
export default function EditPracticeExamPage({
  ...
  // Load exam data + subjects on mount
  useEffect(() => {
    const load = async () => {
      setLoadingSubjects(true)
      try {
        const [examRes, subjectsRes] = await Promise.all([
          fetch(`/api/admin/practice-exams/${params.id}`),
        ...
        const exam = examData.practiceExam
        setTitle(exam.title)
        setDescription(exam.description || '')
        setSubjectId(exam.subject.id)
        setSelectedIds(exam.questions.map((q: any) => q.id))
  ...
  // Reload questions when subjectId changes
  useEffect(() => {
    if (!subjectId) { setAllQuestions([]); return }
    const load = async () => {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true)
    try {
      const res = await fetch(`/api/admin/practice-exams/${params.id}`, {
        method: 'PATCH',
      ...
      toast.success('Practice exam updated successfully!')
      router.push('/admin/practice-exams')
    } catch (err: any) {
      toast.error(err.message || 'Failed to update practice exam')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Practice Exam</h1>
          <p className="text-sm text-gray-500 mt-0.5">Update details and questions</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[340px_1fr] gap-6 items-start">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <Select
                  value={subjectId}
                  onValueChange={v => { setSubjectId(v); setSelectedIds([]) }}
                ...
                {subjectId && (
                  <p className="text-xs text-amber-600 mt-1">
                    ⚠ Changing subject clears selected questions
                  </p>
                )}
              </div>
              ...
              {selectedIds.length === 0 ? (
                ...
                <div className="space-y-1.5 max-h-64 overflow-y-auto">
                  {selectedQuestions.map((q, i) => (
                    ...
                    >
                      <span className="shrink-0 w-5 h-5 rounded-full bg-gray-200 text-gray-600 flex items-center justify-center font-semibold mt-0.5">
                        {i + 1}
                      </span>
                      <p
        ...
        {/* RIGHT — Question Picker (identical to create page) */}
        <Card>
          <CardHeader className="pb-3">
                  ...
                  return (
                    <div
                      ...
                      className={`p-3 cursor-pointer transition-colors ${
                        isSelected ? 'bg-blue-50 hover:bg-blue-100/80' : 'bg-white hover:bg-gray-50'
                      }`}
