...
export default function AdminExamViewPage() {
  ...
  const downloadExamPDF = () => {
    ...
    const questionsHtml = exam.questions.map((q, index) => {
      const optionsHtml = q.options.map((opt) => {
        ...
        const border  = isCorrect ? '2px solid #10b981' : '1px solid #e5e7eb'
      ...
      const diffBg    = q.difficulty === 'easy' ? '#f0fdf4' : q.difficulty === 'medium' ? '#fffbeb' : '#fef2f2'
    ...
    const difficultyBadgeColor = exam.difficulty === 'easy' ? '#16a34a' : exam.difficulty === 'medium' ? '#d97706' : '#dc2626'
    ...
    printWindow.document.write(`
      ...
      <body>
        ...
        <div style="display:flex;gap:12px;margin:0 24px 20px;flex-wrap:wrap;">
          ${[
            { label: 'Total Questions', value: exam.questions.length, color: '#6366f1' },
            { label: 'Total Marks',     value: exam.totalMarks,       color: '#10b981' },
            { label: 'Duration',        value: `${exam.duration} min`, color: '#f59e0b' },
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardContent className="pt-6">
            ...
            <Badge variant={exam.isPublished ? 'default' : 'secondary'} className="mt-2">
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="text-2xl font-bold text-gray-900 mt-1 flex items-center gap-2">
              <Clock className="h-5 w-5" />{exam.duration}min
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="text-2xl font-bold text-gray-900 mt-1">{exam.totalMarks}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="text-2xl font-bold text-gray-900 mt-1 flex items-center gap-2">
              <Users className="h-5 w-5" />{exam.totalAttempts}
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <Badge className={`mt-1 ${getDifficultyColor(exam.difficulty)}`}>{exam.difficulty}</Badge>
            </div>
            <div>
              <p className="text-sm font-medium text-gray-600">Price</p>
              <p className="text-lg font-semibold mt-1">
                {exam.isFree ? <span className="text-green-600">Free</span> : `₹${(exam.price / 100).toFixed(2)}`}
              </p>
            </div>
            <div>
      ...
      {/* Questions List */}
      <Card>
        <CardHeader><CardTitle>Questions ({exam.questions.length})</CardTitle></CardHeader>
        <CardContent>
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <Badge variant="outline" className="text-xs">{question.topic.name}</Badge>
                    ...
                    {question.imageUrl && (
                      <img src={question.imageUrl} alt="Question" className="mt-3 rounded-lg max-w-md" />
                    )}
                  </div>
                  <div className="text-right ml-4">
                    <p className="text-sm text-gray-600">Marks</p>
                    <p className="text-lg font-semibold text-green-600">+{question.marks}</p>
                    {question.negativeMarks > 0 && (
                      <p className="text-sm text-red-600">-{question.negativeMarks}</p>
                    )}
                  </div>
                </div>
                    ...
                    <div key={option.key}
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          {option.imageUrl && (
                            <img src={option.imageUrl} alt={`Option ${option.key}`} className="mt-2 rounded max-w-xs" />
                          )}
                        </div>
                        {option.isCorrect && <CheckCircle2 className="h-5 w-5 text-green-600 flex-shrink-0" />}
                      </div>
                    </div>
                  ))}
                </div>
