...
import { Loader2, User, CheckCircle2, Maximize2, Minimize2, Delete } from 'lucide-react'
...
function NumericalCalculator({
  ...
  questionId: string
  value: string
  onChange: (questionId: string, display: string, numValue: number | null) => void
}) {
  const handleButton = (key: string) => {
    ...
    const isValid = next !== '' && next !== '-' && next !== '.' && !isNaN(parsed)
  ...
  return (
    <div className="flex flex-col items-start gap-4 py-4">
      <div className="w-72 bg-gray-900 text-white rounded-lg p-4 flex items-center justify-between min-h-[56px]">
        <span className="text-2xl font-mono tracking-widest">{value || '___'}</span>
        <button
...
export default function ExamInterface() {
  ...
  const handleSubmit = useCallback(async (autoSubmit = false) => {
    ...
    try {
      ...
      if (lockdownRef.current?.allowSubmission) lockdownRef.current.allowSubmission()
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-600">
                    Exam Name:{' '}
                    <span className="font-semibold text-orange-600 uppercase">{exam.examTitle}</span>
                  </div>
