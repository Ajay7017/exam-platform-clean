...
function getPerformanceLabel(pct: number) {
  ...
  if (pct >= 40)
    return { label: "Average", cls: "bg-yellow-100 text-yellow-700" };
...
function TimeStatsCard({
  ...
  const total =
    timeStats.correct + timeStats.wrong + timeStats.unattempted || 1;
  const blocks = [
    {
      label: "Correct",
      value: timeStats.correct,
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          {blocks.map((b) => (
            <div
              key={b.label}
              className={`flex items-center justify-between rounded-lg px-3 py-2 ${b.bg}`}
            >
        ...
        <p className="text-xs text-gray-400 text-center">
          Total tracked: {formatTime(total)} of {formatTime(totalTime)} exam
          time
        </p>
      </CardContent>
    </Card>
...
function ScorecardCard({ result }: { result: any }) {
  ...
  const you = {
    ...
    percentage: result.percentage?.toFixed(1) || "0",
  };
  ...
  const rows = [
    {
      label: "Score",
      you: `${you.score} / ${result.totalMarks}`,
      topper: topper
        ? `${topper.score.toFixed(1)} / ${result.totalMarks}`
        : null,
      avg: average
        ? `${average.score.toFixed(1)} / ${result.totalMarks}`
        : null,
    },
    {
      label: "Accuracy",
      you: `${you.percentage}%`,
      topper: topper ? `${topper.percentage.toFixed(1)}%` : null,
      avg: average ? `${average.percentage.toFixed(1)}%` : null,
    },
    {
      label: "Correct",
      you: `${you.correct}`,
      topper: topper ? `${Math.round(topper.correct)}` : null,
      avg: average ? `${average.correct.toFixed(1)}` : null,
    },
    {
      label: "Wrong",
      you: `${you.wrong}`,
      topper: topper ? `${Math.round(topper.wrong)}` : null,
      avg: average ? `${average.wrong.toFixed(1)}` : null,
    },
    {
      label: "Time",
      you: formatTime(you.time),
      topper: topper ? formatTime(Math.round(topper.time)) : null,
      avg: average ? formatTime(Math.round(average.time)) : null,
    },
  ];
  return (
    <Card>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b">
                <th className="text-left py-2 text-xs text-gray-400 font-medium w-24">
                  Metric
                </th>
            ...
            <tbody>
              {rows.map((row, i) => (
                ...
                >
                  <td className="py-2 text-xs text-gray-500 font-medium">
                    {row.label}
                  </td>
          ...
          >
            ...
            <Tooltip
              ...
              labelFormatter={(label) => {
                ...
                return entry
                  ? `${label} ${entry.isOfficial ? "(Official)" : "(Practice)"}`
                  : label;
              }}
            />
            <Line
      ...
      <div className="flex items-center justify-center h-64">
        <div className="text-center space-y-3 max-w-sm">
          ...
          <p className="text-xs text-gray-400">
            Results are being processed and will reflect here shortly. You can
            safely leave this page and check back in a few minutes.
          </p>
        </div>
      </div>
    );
...
export default function ResultPage() {
  ...
  const filteredQuestions = (result.questionResults || []).filter((q: any) => {
    ...
    if (solutionFilter === "wrong")
      return !q.isCorrect && q.yourAnswer !== null;
          ...
          q.questionType === "mcq" || q.questionType === "match"
            ? (q.options || [])
                .map((opt: any) => {
                  ...
                  const bg = optIsCorrect
                    ? "#ecfdf5"
                    : optIsYours
                      ? "#fef2f2"
                      : "transparent";
                  const color = optIsCorrect
                    ? "#059669"
                    : optIsYours
                      ? "#dc2626"
                      : "#374151";
                  const border = optIsCorrect
                    ? "2px solid #10b981"
                    : optIsYours
                      ? "2px solid #ef4444"
                      : "1px solid #e5e7eb";
    ...
    <div className="max-w-4xl mx-auto space-y-5 -mt-2">
      ...
      <Card className="overflow-hidden">
        <CardContent className="p-0">
          <div className="flex flex-col sm:flex-row">
            <div
              className={`relative bg-gradient-to-br ${gradient} sm:w-48 h-40 sm:h-auto flex-shrink-0 flex flex-col items-center justify-center gap-2`}
            >
              <div
              ...
              <div>
                ...
                {isPractice && (
                  <p className="text-xs text-blue-600 mt-1">
                    Practice attempt — this score does not affect your rank or
                ...
                {[
                  ...
                  },
                  {
                    ...
                    value: isPractice
                      ? "—"
                      : result.percentile
                        ? `${Number(result.percentile).toFixed(1)}%`
                        : "Updating...",
                  },
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div
            key={label}
            className={`flex items-center gap-3 rounded-lg border px-4 py-3 ${bg}`}
          >
        ...
        <div className="flex items-start gap-3 px-4 py-3 rounded-lg bg-amber-50 border border-amber-200">
          ...
          <div>
            <p className="text-sm font-medium text-amber-800">
              Rank updating within 30 minutes
            </p>
            <p className="text-xs text-amber-600 mt-0.5">
              Your score and answers are final. Rank and percentile are
              calculated together for all students to ensure fair comparison —
              they will appear here within 30 minutes.
            </p>
          </div>
        </div>
      )}
                    ...
                    result.topicWisePerformance.map((topic: any) => (
                      <div key={topic.topic}>
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-1.5">
                          <div
                            className={`h-1.5 rounded-full transition-all ${topic.accuracy >= 75 ? "bg-green-500" : topic.accuracy >= 50 ? "bg-yellow-500" : "bg-red-500"}`}
        ...
        <TabsContent value="leaderboard">
          {isPractice && (
            <div className="flex items-center gap-2 mb-3 px-3 py-2 rounded-lg bg-blue-50 border border-blue-100 text-xs text-blue-700">
              ...
              This was a practice attempt. The leaderboard shows official
              attempts only.
            </div>
          )}
          <LeaderboardCard
              ...
              {(
                [
                  ...
                  },
                  {
                    key: "wrong",
                    label: "Incorrect",
                    count: filterCounts.wrong,
                    activeClass: "bg-red-500 text-white",
                  },
                  {
                    key: "unattempted",
                    label: "Unattempted",
                    count: filterCounts.unattempted,
                ...
                >
                  {f.label}
                  <span
                    className={`text-xs px-1.5 py-0.5 rounded-full ${solutionFilter === f.key ? "bg-white/20" : "bg-gray-100"}`}
                  >
                    {f.count}
                  </span>
                </button>
          ...
          {filteredQuestions.length === 0 ? (
            ...
            filteredQuestions.map((q: any) => {
              ...
              return (
                <Card
                  key={q.questionId}
                  className={`border-l-4 ${q.isCorrect ? "border-l-green-500" : q.yourAnswer ? "border-l-red-500" : "border-l-gray-300"}`}
                >
                  <CardHeader className="pb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`text-xs text-gray-400 ${q.timeSpentSec > 0 ? "" : "ml-auto"}`}
                      >
                        {q.isCorrect
                          ? `+${q.marks}`
                          : q.yourAnswer
                  ...
                  <CardContent className="space-y-3 pt-0">
                    {q.questionType === "mcq" && (
                      <div className="space-y-1.5">
                        {q.options?.map((opt: any) => (
                          <div
                            key={opt.key}
                            className={`flex items-center gap-2 p-2.5 rounded-lg border text-sm ${opt.isCorrect ? "border-green-400 bg-green-50" : opt.key === q.yourAnswer && !q.isCorrect ? "border-red-400 bg-red-50" : "border-gray-200"}`}
                          >
                            <span className="font-semibold text-gray-500 w-4">
                              {opt.key}.
                            </span>
                    ...
                    {q.questionType === "numerical" && (
                      <div className="grid grid-cols-2 gap-3">
                        <div
                          className={`p-3 rounded-lg border-2 ${q.yourAnswer ? (q.isCorrect ? "border-green-400 bg-green-50" : "border-red-400 bg-red-50") : "border-gray-200 bg-gray-50"}`}
                        >
                          <p className="text-xs text-gray-500 mb-1">
                            Your Answer
                          </p>
                          <p
                            className={`text-xl font-bold ${q.yourAnswer ? (q.isCorrect ? "text-green-700" : "text-red-700") : "text-gray-400"}`}
                          >
