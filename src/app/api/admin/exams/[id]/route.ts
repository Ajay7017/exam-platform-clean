...
export async function GET(
  ...
  try {
          ...
          include: {
            question: {
              include: {
                ...
                // ✅ include matchPairs for match type questions
              },
            },
          },
          orderBy: { sequence: "asc" },
    ...
    const questions = exam.questions.map((eq) => ({
      ...
      questionType: eq.question.type ?? "mcq", // ✅ added
      topic: {
      ...
      matchPairs: eq.question.matchPairs ?? null, // ✅ added
    }));
...
export async function PUT(
  ...
  try {
    ...
    if (validated.randomizeOrder !== undefined)
      updateData.randomizeOrder = validated.randomizeOrder;
    if (validated.allowReview !== undefined)
      updateData.allowReview = validated.allowReview;
  ...
  try {
    ...
    if (exam._count.attempts > 0 || exam._count.purchases > 0) {
      return NextResponse.json(
        {
          error:
            "Cannot delete exam with existing attempts or purchases. Unpublish it instead.",
        },
        { status: 400 },
      );
    }
